...
import { ACE, RANKS, RANK_LABELS_NL, SUITS } from '@shared/core/types.ts';
import { cardId } from '@shared/core/deck.ts';
...
function drawCornerIndex(
  ...
  rotated: boolean,
  rechts = false,
  labels: Readonly<Record<Rank, string>> = RANK_LABELS_NL,
): void {
  ctx.save();
  if (rotated) {
    ctx.translate(w, h);
...
export function drawCardFace(
  ...
  drawCornerIndex(ctx, card, w, h, false, false, labels);
  drawCornerIndex(ctx, card, w, h, false, true, labels);
  drawCornerIndex(ctx, card, w, h, true, false, labels);
  drawCornerIndex(ctx, card, w, h, true, true, labels);
